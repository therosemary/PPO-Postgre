...
class PPO2:
    ...
    def learn(self, total_timesteps):
        ...
        batch_obs = torch.zeros(
            (self.timesteps_per_batch, self.num_envs) + self.envs.single_observation_space.shape).to(self.device)
        batch_acts = torch.zeros((self.timesteps_per_batch, self.num_envs) + self.envs.single_action_space.shape).to(
            self.device)
